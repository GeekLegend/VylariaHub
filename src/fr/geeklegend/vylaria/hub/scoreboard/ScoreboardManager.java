...
import fr.vylaria.api.servers.ServerNotFoundException;
...
public class ScoreboardManager
{
    ...
    {
        scoreboards = new HashMap<UUID, PersonalScoreboard>();
        ipCharIndex = 0;
        cooldown = 0;

        glowingTask = VylariaHub.getInstance().getScheduledExecutorService().scheduleAtFixedRate(() ->
        {
            String ip = colorIpAt();
            for (PersonalScoreboard scoreboard : scoreboards.values())
                VylariaHub.getInstance().getExecutorMonoThread().execute(() ->
                {
        ...
        {
            for (PersonalScoreboard scoreboard : scoreboards.values())
                VylariaHub.getInstance().getExecutorMonoThread().execute(scoreboard::reloadData);
